...
- 例如，当你在 Facebook、Twitter 或 微信 等社交平台上分享你的博客或网站时，平台会自动抓取该网页的一些信息（如标题、描述、图片等）。你可以通过设置 social_preview_image 来指定一个默认的图片，这张图片就会在分享链接时显示。
...
## Giscus
- Giscus 是一个基于 GitHub Discussions 的评论系统，它为每个页面创建一个 GitHub Discussions 话题，可以让用户在页面下方直接评论。 
- 配置项说明： 
...
- 这个选项是一个布尔值，用来开启或关闭自托管静态资源的功能。 
  - 如果 enabled 为空或为 false，表示不启用自托管静态资源，Jekyll 会默认使用其他设置的资源托管方式（例如 CDN 或直接使用相对路径）。 
...
- 这个选项允许你指定资源托管的环境类型，决定在哪个环境下使用自托管的静态资源。 
  ...
  - 如果设置为 development 或 production，表示根据 Jekyll 的构建环境来选择是否使用自托管资源。比如在开发环境下可能使用本地文件，在生产环境下使用服务器上的文件或路径。
...
- 这个配置项的作用是限制每个页面显示的文章数量为 10 篇。如果你有超过 10 篇文章，Jekyll 会自动生成分页系统，用户可以点击查看下一页的文章。
...
- 这个配置项是用来设置网站的基础 URL。
- 如果你的网站是托管在 GitHub Pages 的根目录(例如 https://username.github.io/), 那么 baseurl 应该设置为空字符串 `""`。
- 如果你的网站是托管在子目录下(例如 https://username.github.io/repository-name/),那么 baseurl 就需要设置为 `/repository-name`。
